...
class DosenInput extends PolymerElement{
    static get template(){
        return html`
            ...
            <app-header-layout>
                <app-header slot = "header" fixed condenses effects="waterfall">
                    <app-toolbar>
                        ...
                        <div main-title>Kelola Data Dosen</div>
                    </app-toolbar>
                </app-header>
                <div class= "card">
                    <h3>Masukkan Data</h3>
                    <paper-input name="id" class="input-form" label="NIK Dosen" value="[[data.id]]"></paper-input>
                    <paper-input name="nama" class="input-form" label="Nama Dosen" value="[[data.nama]]"></paper-input>
    ...
    _computeView(route,page){
        ...
        if (path[path.length-1] !== 0) {
          this.shadowRoot.querySelector("#ajax").url = MyAppGlobals.apiPath + '/api/v1/bootcamp/dosen/' + path[path.length-1]
      ...
      _clear(){
        window.history.pushState({},null,'/ubahdosen/0');
      ...
      _add(){
        ...
        for(let i=0;i<inputs.length;i++){
            ...
            this.shadowRoot.querySelector("#ajaxPostPut").url = MyAppGlobals.apiPath + '/api/v1/bootcamp/dosen' + dataID[dataID.length-1];
            this.shadowRoot.querySelector("#ajaxPostPut").method = "PUT";
            this.shadowRoot.querySelector("#ajaxPostPut").body = JSON.stringify(data)
            inputs[i].value = ""
            }else{ //INSERT
            this.shadowRoot.querySelector("#ajaxPostPut").url = MyAppGlobals.apiPath + '/api/v1/bootcamp/dosen' ;
    ...
    _delete(e){
        ...
        if (path[path.length-1] !== 0) {
          this.shadowRoot.querySelector("#ajaxDelete").url = MyAppGlobals.apiPath + '/api/v1/bootcamp/dosen/' + path[path.length-1]
    ...
    _closeTab(){
        window.history.pushState({},null,'/listdosen');
        window.dispatchEvent(new CustomEvent('location-changed'));
    }
    ...
    let data = e.detail.response.data;
    this.set('data',data);
   //window.history.pushState({}, null, '/ubahdosen/'+data['npm']);
    window.history.pushState({}, null, '/listdosen/');
    window.dispatchEvent(new CustomEvent('location-changed'));
    }
...
window.customElements.define('dosen-input', DosenInput);